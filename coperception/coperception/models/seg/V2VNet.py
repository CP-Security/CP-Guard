...
class V2VNet(SegModelBase):
    ...
    def forward(self, x, trans_matrices, num_agent_tensor, pert=None, eps=None, attacker_list=None, collab_agent_list=None, ego_agent=None, no_fuse=False, adv_method=None):
        x1 = self.inc(x)
        x2 = self.down1(x1)
        x3 = self.down2(x2)
                    ...
                    for j in range(com_num_agent):
                        if j != i:
                            ...
                            if pert is not None and j in attacker_list:
                                # print(j,torch.isnan(pert[j]).any(), pert[j])
                                # clip
                                if adv_method == 'fgsm':
                            ...
                            if collab_agent_list is not None and j not in collab_agent_list:
                                continue
